...
void draw() {
  ...
  float posXtwo = rthumb1;
  float posYtwo = rthumb2;
  float circleTwoRadius = random(5, 15) + 5;
  
  float posXth = rthumb1;
  float posYth = rthumb2;
  ...
  if (frameCount % 4 == 0 ) {
    tx = posX + lthumb2 * random(-.2, .2);
    ty = posY + lthumb1 * random(-.5, .5);

    txtwo = posXtwo + lthumb1 * random(-.2, .2);
    tytwo = posYtwo + lthumb2 * random(-.5, .5);
    
    txth = posXth + lthumb1 * random(-.4, .4);
    tyth = posYth + lthumb2 * random(-.8, .8);
  }
